...
namespace ftl { namespace wasm_validations {
using namespace IR;
...
void memories_validation_visitor::validate( const Module& m ) {
   if ( m.memories.defs.size() && m.memories.defs[0].type.size.min > wasm_constraints::maximum_linear_memory/(64*1024) )
      FTL_THROW(wasm_runtime_exception, "Smart contract initial memory size must be less than or equal to ${0}KiB",
            wasm_constraints::maximum_linear_memory/1024);
}
...
void data_segments_validation_visitor::validate(const Module& m ) {
   for ( const DataSegment& ds : m.dataSegments ) {
      if ( ds.baseOffset.type != InitializerExpression::Type::i32_const )
         FTL_THROW( wasm_runtime_exception, "Smart contract has unexpected memory base offset type" );

      if ( static_cast<uint32_t>( ds.baseOffset.i32 ) + ds.data.size() > wasm_constraints::maximum_linear_memory_init )
         FTL_THROW(wasm_runtime_exception, "Smart contract data segments must lie in first ${0}KiB",
               wasm_constraints::maximum_linear_memory_init/1024);
   }
}
...
void tables_validation_visitor::validate( const Module& m ) {
   if ( m.tables.defs.size() && m.tables.defs[0].type.size.min > wasm_constraints::maximum_table_elements )
      FTL_THROW(wasm_runtime_exception, "Smart contract table limited to ${0} elements",
            wasm_constraints::maximum_table_elements);
}
...
void globals_validation_visitor::validate( const Module& m ) {
   ...
   for(const GlobalDef& global_def : m.globals.defs) {
      ...
      switch(global_def.type.valueType) {
         case ValueType::any:
         case ValueType::num:
            FTL_THROW(wasm_runtime_exception, "Smart contract has unexpected global definition value type");
         case ValueType::i64:
         case ValueType::f64:
   ...
   if(mutable_globals_total_size > wasm_constraints::maximum_mutable_globals)
      FTL_THROW(wasm_runtime_exception, "Smart contract has more than ${0} bytes of mutable globals",
            wasm_constraints::maximum_mutable_globals);
}
...
void maximum_function_stack_visitor::validate( const IR::Module& m ) {
   for(const FunctionDef& func : m.functions.defs) {
      ...
      if(function_stack_usage > wasm_constraints::maximum_func_local_bytes)
         FTL_THROW(wasm_runtime_exception, "Smart contract function has more than ${0} bytes of stack usage",
            wasm_constraints::maximum_func_local_bytes);
   }
}
...
void ensure_apply_exported_visitor::validate( const IR::Module& m ) {
   ...
   if(!found_it)
      FTL_THROW(wasm_runtime_exception, "Smart contract's apply function not exported; non-existent; or wrong type");
}
