...
namespace ftl {
    namespace wasm_validations {
        ...
        struct large_offset_validator {
            ...
            static void accept(wasm_ops::instr *inst, wasm_ops::visitor_arg &arg) {
                ...
                if (memarg_instr->field.o >= wasm_constraints::maximum_linear_memory)
                    FTL_THROW(wasm_runtime_exception, "Smart contract used an invalid large memory store/load offset");
            }
        ...
        struct blacklist_validator {
            ...
            static void accept(wasm_ops::instr *inst, wasm_ops::visitor_arg &arg) {
                FTL_THROW(wasm_runtime_exception, "Error, blacklisted opcode ${0} ", inst->to_string());
            }
        };
        ...
        struct nested_validator {
            ...
            static void accept(wasm_ops::instr *inst, wasm_ops::visitor_arg &arg) {
                if (!disabled) {
                    ...
                    depth++;
                    FTL_ASSERT(depth < 1024, wasm_runtime_exception, "Nested depth exceeded");
                }
            }
        };
