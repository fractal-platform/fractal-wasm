...
namespace ftl { namespace wasm_ops {
...
// using for instructions with no fields
    ...
    inline void pack(instruction_stream *stream, branchtabletype field) {
        const char packed[] = {char(field.target_depth), char(field.target_depth >> 8), char(field.target_depth >> 16),
                               char(field.target_depth >> 24),
                               char(field.target_depth >> 32), char(field.target_depth >> 40),
                               char(field.target_depth >> 48), char(field.target_depth >> 56),
                               char(field.table_index), char(field.table_index >> 8), char(field.table_index >> 16),
                               char(field.table_index >> 24),
    ...
    struct field_specific_params {
        ...
        static void pack(instruction_stream *stream, Field &f) { return wasm_ops::pack(stream, f); }

        static auto to_string(Field &f) {
            return std::string(" ") +
                   wasm_ops::to_string(f);
        }
    };
...
struct OP final : instr_base<Mutators...> {                                                         \
   ...
   std::string to_string() override {                                                               \
      return std::string(#OP)+field_specific_params<DATA>::to_string( field );   \
   }                                                                                                \
};
...
//CONSTRUCT_OP_HAS_DATA(0, voidtype, error)
...
// This code is from wasm-jit/Include/IR/Operators.h
    ...
    struct EOSIO_OperatorDecoderStream {
        ...
        instr *decodeOp() {
            FTL_ASSERT(nextByte + sizeof(IR::Opcode) <= end, wasm_runtime_exception, "");
            IR::Opcode opcode = *(IR::Opcode *) nextByte;
            switch (opcode) {
#define VISIT_OPCODE(opcode, name, nameString, Imm, ...) \
         case IR::Opcode::name: \
         { \
            FTL_ASSERT(nextByte + sizeof(IR::OpcodeAndImm<IR::Imm>) <= end, wasm_runtime_exception, ""); \
